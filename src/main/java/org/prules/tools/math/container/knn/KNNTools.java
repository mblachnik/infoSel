...
public class KNNTools {
     ...
     * @param collection NearestNeighbor structure
     * @return
     */
    @Deprecated
    public static double predictOneNearestNeighbor(Example example, ISPRGeometricDataCollection<IInstanceLabels> collection) {
        Collection<IInstanceLabels> resultSet = collection.getNearestValues(1, InstanceFactory.createVector(example));
    ...
    public static double predictOneNearestNeighbor(Vector instance, List<Vector> samples, List<IInstanceLabels> labels, DistanceMeasure distance, IDataIndex index) {
        ...
        return bestLabel != null ? bestLabel.getLabel() : Double.NaN;
    }
    ...
    public static void doNNVotes(double[] votes, Vector values, ISPRGeometricDataCollection<IInstanceLabels> samples, int k, VotingType voting) {
        ...
        switch (voting) {
            ...
            case GAUSSIAN:
                ...
                for (DoubleObjectContainer<IInstanceLabels> tuple : neighbours) {
                    //totalDistance += tuple.getFirst();
                    totalDistance = Math.max(totalDistance, tuple.getFirst());
                }
                ...
                switch (voting) {
                    ...
                    case LINEAR:
                        for (DoubleObjectContainer<IInstanceLabels> tuple : neighbours) {
                            int idx = (int) tuple.getSecond().getLabel();
                            votes[idx] += (totalDistance - tuple.getFirst()) / totalDistance;
                        }
                    ...
                    case GAUSSIAN:
                        for (DoubleObjectContainer<IInstanceLabels> tuple : neighbours) {
                            int idx = (int) tuple.getSecond().getLabel();
                            double res = (tuple.getFirst() / totalDistance);
    ...
    public static double getRegVotes(Vector values, ISPRGeometricDataCollection<IInstanceLabels> samples, int k, VotingType weighting) {
        ...
        switch (weighting) {
            ...
            case MAJORITY:
                ...
                for (IInstanceLabels neighbourLabel : neighbourLabels) {
                    double nearestOutput = neighbourLabel.getLabel();
            ...
            case EXPONENTIAL:
                ...
                for (DoubleObjectContainer<IInstanceLabels> tuple : neighbours) {
                    double nearestOutput = tuple.getSecond().getLabel();
                    double distance = tuple.getFirst();
                    if (distance == 0) {
                        predictedValue = nearestOutput;
                        totalSimilarity = 1;
                        break;
