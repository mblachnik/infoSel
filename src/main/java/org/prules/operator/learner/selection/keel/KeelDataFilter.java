 ...
 * @author Marcin
 */
class KeelDataFilter {

    private Map<String, ArrayList<Integer>> map;
    private int n, m;
    private int numberOfMisses;

    KeelDataFilter(double[][] trainingSet, int[] labels) {
        numberOfMisses = 0;
        n = trainingSet.length;
        m = n > 0 ? trainingSet[0].length : -1;
        map = new HashMap<>(n); //Map used to identify identical rows - here we have List<Integer> because two rows may be identical, each with different id
        for (int i = 0; i < n; i++) {
    ...
    public IDataIndex filterSamples(double[][] trainingSet, int[] labels) {
        ...
        for (int i = 0; i < trainingSet.length; i++) {
            ...
            if (list != null) {
                if (!list.isEmpty()) {
                    list.remove(0); //Remove first occurrence
