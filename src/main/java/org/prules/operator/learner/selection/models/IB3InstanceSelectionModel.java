...
public class IB3InstanceSelectionModel extends AbstractInstanceSelectorModel {
    ...
    private int k;
    private double upperInterval, lowerInterval;
    private transient double[] tmpClassFrequency; //temporal variable declared here to avoid reallocation
    ...
    public DataIndex selectInstances(SelectedExampleSet exampleSet) {
        ...
        BoundedPriorityQueue<Tupel<Double, Integer>> nearestAcceptable = new BoundedPriorityQueue<>(k);
        //BoundedPriorityQueue<Tuple<Double, Integer>> nearestOther      = new BoundedPriorityQueue<>(k);
        double maxDist;
        ...
        for (Example example : trainingSet) {
            ...
            //Search for nearest acceptable instances in selectedSet. If a selected instance is not acceptable then it marked as a member of notAcceptableSet
            for (Example selectedExample : selectedSet) {
                ...
                if ((instanceTrialCounter[i] > 0) && (minConfidence(instanceSuccessCounter[i], instanceTrialCounter[i], upperInterval) > classMinConfidence[labelID])) {
                    ...
                    //nearestOther.add(new Tuple<Double,Integer>(dist, labelID));
                }
            }
            //If number of acceptable instances is smaller then "k", then we search by random in notAcceptableSet and add as many as necessary to meet "k" neighbors
            int otherSize;
            ...
            //From selectedSet we check if any could be removed by applying the lower bound of confidence interval
            for (Example selectedExample : selectedSet) {
                i = ((ISPRExample) selectedExample).getIndex();
    ...
    private Tupel<Double, Double> predictLabelAndMaxDist(BoundedPriorityQueue<Tupel<Double, Integer>> nearestAcceptable) {
        ...
        for (Tupel<Double, Integer> tuple : nearestAcceptable) {
            double dist = tuple.getFirst();
            if (dist > maxDist) {
                maxDist = dist;
            }
            int tmpLabel = tuple.getSecond();
        ...
        return new Tupel<>(maxDist, mostFreqClass);
    }
