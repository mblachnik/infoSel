...
public abstract class AbstractISDecisionFunction implements IISDecisionFunction {
    /**
     * Function used to define metadata requirements for a loss function. It derives from the fact that some of the
     * decision function require nominal label, some other numerical label, some may need some additional checks.
     * By overriding this method you can define your own meta data requirements.
     * Each first level list represent single error or warning, than in the second level string the first element defines
     * type error/warning than the second parameter
     ...
     * @return list of lists of strings - where in the first order list each element represent single error or warning  entry,
     * than in the second list each string represent 1) type of error/warning, 2) i18nKey, 3..) parameters
     */
    @Override
