...
public class VDMNominal2NumericalBuilderModel implements PRulesModel<VDMNominal2NumericalModel> {
    ...
    public VDMNominal2NumericalModel run(ExampleSet exampleSet) {
        Map<String, PairContainer<double[][],List<IntObjectContainer<String>>>> attributeTransofrmationMap = new HashMap<>(attributeNames.length);
        ...
        if (label.isNominal()) {
            ...
            for (String name : attributeNames) {
                ...
                if (attr != null && attr.isNominal()) {
                    ...
                    //We need the pair of String and associaed value id in case in the example set in which we applie the transformation the symbolic values are associated to diffferent numerical values
                    for(String val : vals){
                        mappings.add(new IntObjectContainer<>(attr.getMapping().getIndex(val),val));
                    }
                    //Get the transofrmation matrix
                    double[][] out = processAttribute(exampleSet, attr,c,n);
                    //collect all togater - the attribute name, transformation matrix and attribute value - int pairs
                    attributeTransofrmationMap.put(name, new PairContainer<>(out,mappings));
                }
            }
        } 
        return new VDMNominal2NumericalModel(attributeTransofrmationMap,labelNames);
    }
