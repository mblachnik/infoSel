...
public class FlattenByDistanceClusterModel extends Operator {
    ...
    public FlattenByDistanceClusterModel(OperatorDescription description) {
        super(description);
    ...
    public ClusterModel flatten(HierarchicalClusterModel model, ExampleSet exampleSet) throws OperatorException {
        ...
        while (queue.size() > 0) {
            HierarchicalClusterNode topNode = queue.peek();
            nodeDistance = topNode.getDistance();
        ...
        queue.addAll(leafs);
        int numberOfClusters = queue.size();
    ...
    public List<ParameterType> getParameterTypes() {
        ...
        ParameterType type = new ParameterTypeDouble(PARAMETER_MAXIMUM_DISTANCE, "Specifies the maximum distance threshold.", 0.0, Double.MAX_VALUE);
