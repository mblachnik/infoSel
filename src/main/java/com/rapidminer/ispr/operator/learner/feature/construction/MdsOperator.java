...
public class MdsOperator extends Operator {
	...
	private final InputPort exampleSetInput = getInputPorts().createPort("example set"); //Dataset input port
	private final OutputPort exampleSetOutput = getOutputPorts().createPort("example set"); //Dataset Output port
	private final DistanceMeasureHelper measureHelper = new DistanceMeasureHelper(this); //Distance Measure

	public static final String PARAMETER_SCALING_ALGORITHM = "kind of scaling"; //Parameter string of the operator: type of MDS algorithm
	public static final String PARAMETER_SCALING_DIMENSIONS = "dim"; //Parameter string of the operator: number of putput dimensions
	public static final String PARAMETER_ORIGINALS = "keep originals"; //Parameter string of the operator: boolean to keep original features

	public static final String ATTRIBUTES_PREFIX = "MDSattr"; //Prefic of new attributes which are added to the example set
	...
	private double[][] generateDistancesMatrix(ExampleSet exampleSet) throws OperatorException {
         ...
         * @return rectangular cooridinates matrix, where number of rows is equal 
