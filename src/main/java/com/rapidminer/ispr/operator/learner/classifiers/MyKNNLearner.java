...
public class MyKNNLearner extends AbstractLearner {
    ...
    public Model learn(ExampleSet exampleSet) throws OperatorException {
        ...
        if (exampleSet.getAttributes().getLabel().isNominal()) {
            ...
            return new MyKNNClassificationModel<Number>(exampleSet, samples, getParameterAsInt(PARAMETER_K), weightingType, PredictionType.Classification);
        }
        if (exampleSet.getAttributes().getLabel().isNumerical()) {
            ISPRGeometricDataCollection<Number> samples = KNNTools.initializeKNearestNeighbourFactory(nnSearchType, exampleSet, measure);
            return new MyKNNClassificationModel<Number>(exampleSet, samples, getParameterAsInt(PARAMETER_K), weightingType, PredictionType.Regression);
        }
