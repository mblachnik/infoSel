...
import com.rapidminer.ispr.operator.learner.optimization.supervised.LVQ2Model;
...
import com.rapidminer.ispr.operator.learner.optimization.supervised.LVQNeighborhoodTypes;
import com.rapidminer.ispr.operator.learner.optimization.supervised.LVQTypes;
import com.rapidminer.ispr.operator.learner.optimization.supervised.SNGModel;
import com.rapidminer.ispr.operator.learner.optimization.supervised.WTMLVQModel;
...
public class LVQOperator extends AbstractPrototypeOptimizationChain {
     ...
     * input training set and initial  codebooks. The codebooks are directly modified, 
     * so it cannot be directly delivered examples from output of another operator. This must be new ExampleSet
     * 
     * @param trainingSet - training ExampleSet
     * @param codebooks - initial Codebooks position
     * @return - kNN model initialized with codebooks. If model output port of the operator is not connected it returns null
    ...
    public MyKNNClassificationModel<Number> optimize(ExampleSet trainingSet, ExampleSet codebooks) throws OperatorException {
        ...
        if (this.modelOutputPort.isConnected()){
            ...
            MyKNNClassificationModel<Number> model = new MyKNNClassificationModel<Number>(codebooks, knn, 1, VotingType.MAJORITY, PredictionType.Classification);
            return model;
        }
        return null;
    }
    ...
    /**
     * This method defines all possible input data properties required by LVQ operator
     * @param capability
     * @return 
     */
    @Override
    ...
    public List<ParameterType> getParameterTypes() {
        ...
        ParameterType type = new ParameterTypeCategory(PARAMETER_LVQ_TYPE, "Defines on type of lvq algorithm.", LVQTypes.typeNames(), 0);
